...
test('updates recurring expense status', async ({ page }) => {
  ...
  await recurringDialog.locator('#recurring-name').fill('Smoke recurring');
  await recurringDialog.locator('#recurring-amount').fill('20');
  await recurringDialog.locator('#recurring-due-day').fill('1');
  await recurringDialog.locator('button[type="submit"]').click();
  const recurringRow = page.locator('#recurring-page .recurring-row').filter({ hasText: 'Smoke recurring' });
  await expect(recurringRow).toBeVisible({ timeout: 15000 });
  await expect(recurringDialog).toBeHidden();
  const recurringStatus = recurringRow.getByRole('combobox', { name: /Smoke recurring/ });
  await recurringStatus.selectOption('paid');
  await expect(recurringStatus).toHaveValue('paid');
});
