<script lang="ts">
  ...
  import { updateMonthlyTask, deleteMonthlyTask } from '../../lib/services/firebase/tasks';
  ...
  function handleAddTask() {
    uiStore.openModal('monthlyTask');
  }
  
  // Handle edit task
  function handleEditTask(taskId: string) {
    uiStore.openModal('monthlyTask', { editId: taskId });
  }
  ...
  async function handleStatusChange(taskId: string, event: Event) {
    ...
    try {
      await updateMonthlyTask(userId, selectedMonthId, taskId, { status: newStatus });
    } catch (error) {
  ...
  const debouncedNotesUpdate = debounce(async (taskId: string, notes: string) => {
    ...
    try {
      await updateMonthlyTask(userId, selectedMonthId, taskId, { notes: notes.trim() || undefined });
    } catch (error) {
  ...
  async function handleDeleteTask(taskId: string) {
    ...
    try {
      await deleteMonthlyTask(userId, selectedMonthId, taskId);
    } catch (error) {
...
<div class="monthly-tasks-section">
  <!-- Section header -->
  <div class="section-header">
    <h3 class="section-title">
      {currentTranslations['monthly_tasks'] || 'Monthly Tasks'}
    </h3>
    <div class="header-controls">
      <span class="month-display">{selectedMonthId}</span>
      <button
        class="btn-add"
  ...
  {#if monthlyTasks.length === 0}
    <EmptyState
      icon="📅"
      message={currentTranslations['no_monthly_tasks'] || 'No tasks for this month'}
      description={currentTranslations['no_monthly_tasks_desc'] || 'Add a task to get started'}
    />
  {:else}
            ...
            <tr class="task-row" class:carried-over={task.carriedOver}>
              <td class="col-name" data-label={currentTranslations['task_name'] || 'Task'}>
                <div class="task-name-cell">
                  ...
                  {#if task.carriedOver && task.originalMonth}
