<script lang="ts">
  ...
  import { updateDailyTask, deleteDailyTask } from '../../lib/services/firebase/tasks';
  import { debounce } from '../../lib/utils/performance';
  import { formatISODateForDisplay } from '../../lib/utils/formatting';
  ...
  $: filteredTasks = dailyTasks.filter(task => task.date === currentDailyDate);
  ...
  function handleDateChange(event: Event) {
    ...
    tasksStore.setCurrentDailyDate(target.value);
  }
  ...
  function handleEditTask(taskId: string) {
    uiStore.openModal('dailyTask', { editId: taskId });
  }
  ...
  async function handleStatusChange(taskId: string, event: Event) {
    ...
    try {
      await updateDailyTask(userId, taskId, { status: newStatus });
    } catch (error) {
  ...
  const debouncedNotesUpdate = debounce(async (taskId: string, notes: string) => {
    ...
    try {
      await updateDailyTask(userId, taskId, { notes: notes.trim() || undefined });
    } catch (error) {
  ...
  async function handleDeleteTask(taskId: string) {
    ...
    try {
      await deleteDailyTask(userId, taskId);
    } catch (error) {
...
<div class="daily-tasks-section">
  ...
  <div class="section-header">
    <h3 class="section-title">
      {currentTranslations['daily_tasks'] || 'Daily Tasks'}
    </h3>
    <div class="header-controls">
      <input
        type="date"
        value={currentDailyDate}
        on:change={handleDateChange}
        class="date-picker"
        aria-label={currentTranslations['select_date'] || 'Select date'}
      />
      <button
        class="btn-add"
  ...
  {#if filteredTasks.length === 0}
    <EmptyState
      icon="📝"
      message={currentTranslations['no_daily_tasks'] || 'No tasks for this date'}
      description={currentTranslations['no_daily_tasks_desc'] || 'Add a task to get started'}
    />
  {:else}
    <div class="tasks-table-wrapper">
      ...
      >
        <table class="tasks-table tasks-table-body">
          <tbody>
            <tr class="task-row" class:carried-over={task.carriedOver}>
              <td class="col-name" data-label={currentTranslations['task_name'] || 'Task'}>
                <div class="task-name-cell">
                  <span class="task-name">{task.name}</span>
                  {#if task.carriedOver && task.first_carry_date}
                    <span class="carried-over-badge" title={currentTranslations['carried_over_from'] || 'Carried over from'}>
                      📅 {formatISODateForDisplay(task.first_carry_date, language)}
                    </span>
                  {/if}
                </div>
              </td>
...
<style>
  ...
  .tasks-table tbody tr.carried-over {
    background: rgba(59, 130, 246, 0.05);
  }
  
  .tasks-table tbody tr.carried-over:hover {
  ...
  :global(.dark) .tasks-table tbody tr.carried-over {
    background: rgba(59, 130, 246, 0.1);
  }
  
  :global(.dark) .tasks-table tbody tr.carried-over:hover {
  ...
  :global(.dark) .carried-over-badge {
    color: #60a5fa;
  ...
  @media (max-width: 768px) {
    ...
    :global(.dark) .task-row.carried-over {
      border-color: rgba(59, 130, 246, 0.3);
    }
