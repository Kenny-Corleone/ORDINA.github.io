<script lang="ts">
  ...
  onMount(() => {
    ...
    loadStocks();
    interval = setInterval(() => loadStocks(), 3 * 60 * 1000); // 3 mins
  });
  ...
  <div class="header-band p-4 flex items-center justify-between border-b border-white/10">
    <div class="flex items-center gap-3">
      ...
      <h3 class="text-xs font-black uppercase tracking-[0.2em] text-white/90">
        MARQUETS <span class="text-white/30 font-light">Live</span>
      </h3>
    </div>
    <button
    ...
    {:else if error && stocks.length === 0}
      <div class="flex flex-col items-center justify-center py-10 opacity-40">
        <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
          ...
          /></svg
        >
        <span class="text-[10px] uppercase font-bold">{error}</span>
      </div>
    {:else}
            ...
            <div class="flex flex-col items-end gap-0.5">
              ...
              >
                <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  {#if stock.change >= 0}
                    <path d="M5 15l7-7 7 7" stroke-width="3" />
                  {:else}
