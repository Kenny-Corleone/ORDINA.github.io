...
import { exportExpensesToCSV, exportDebtsToCSV } from './export.js';
...
function updateDashboard() {
     ...
     // Sync news height with recent activity container
     syncNewsHeight();
...
function renderDashboardTasks(tasks) {
     ...
     list.innerHTML = activeTasks.slice(0, 5).map(t => `
        <div class="flex items-center justify-between p-2.5 bg-gray-50 dark:bg-gray-800/40 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-all cursor-pointer border border-transparent hover:border-blue-500/30" onclick="document.querySelector('[data-tab=\\'tasks\\']').click()">
            <span class="truncate pr-2 text-[13px] font-medium">${t.name}</span>
            <span class="text-[10px] px-2 py-0.5 bg-blue-100 text-blue-800 rounded-md dark:bg-blue-900/50 dark:text-blue-200 font-bold uppercase tracking-wider">Today</span>
        </div>
    `).join('');
}
...
function updateDashboardClock() {
     ...
     const update = () => {
          const now = new Date();
          timeEl.textContent = now.toLocaleTimeString(currentLang === 'ru' ? 'ru-RU' : currentLang === 'az' ? 'az-Latn-AZ' : 'en-US', { hour: '2-digit', minute: '2-digit' });
          dateEl.textContent = now.toLocaleDateString(currentLang === 'ru' ? 'ru-RU' : currentLang === 'az' ? 'az-Latn-AZ' : 'en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
     };
...
function updateRecentActivity() {
     ...
     // Sync news height after activity is updated
     setTimeout(syncNewsHeight, 100);
}
...
function setupEventListeners() {
     ...
     if (exportExpensesBtn) {
          exportExpensesBtn.addEventListener('click', () => {
               try {
                    exportExpensesToCSV(allExpenses, `expenses-${selectedMonthId}.csv`);
     ...
     if (exportDebtsBtn) {
          exportDebtsBtn.addEventListener('click', () => {
               try {
                    exportDebtsToCSV(allDebts, 'debts.csv');
                    showToast('Debts exported successfully', 'success');
               } catch (e) {
                    logger.error('Export error:', e);
                    showToast('Failed to export debts', 'error');
               }
          });
     }
     ...
     // Language Flags
     const flagByLang = { ru: '🇷🇺', en: '🇬🇧', az: '🇦🇿', it: '🇮🇹' };
     const titleByLang = { ru: 'Русский', en: 'English', az: 'Azərbaycan', it: 'Italiano' };
     ...
     // Dashboard Tasks Panel Toggle
     const toggleTasksPanel = document.getElementById('toggle-tasks-panel');
     const tasksPanel = document.getElementById('dashboard-tasks-panel');
     const tasksArrow = document.getElementById('tasks-panel-arrow');

     if (toggleTasksPanel && tasksPanel && tasksArrow) {
          toggleTasksPanel.addEventListener('click', () => {
               const isCollapsed = tasksPanel.style.maxHeight === '50px';
               if (isCollapsed) {
                    tasksPanel.style.maxHeight = '20vh';
                    tasksArrow.style.transform = 'rotate(0deg)';
               } else {
                    tasksPanel.style.maxHeight = '50px';
                    tasksArrow.style.transform = 'rotate(-90deg)';
               }
          });
     }
}
...
async function handleExpenseForm(form) {
     ...
     const amount = parseFloat(form.querySelector('#expense-amount').value) || 0;
     const data = {
          name: form.querySelector('#expense-name').value.trim(),
...
async function handleDebtPaymentForm(form) {
     ...
     const amount = parseFloat(form.querySelector('#debt-payment-amount').value) || 0;
...
function syncNewsHeight() {
     ...
     if (recentActivityContainer && newsWidget && newsResults && newsToolbar) {
          // Wait for layout to settle - use multiple frames for accuracy
          requestAnimationFrame(() => {
               requestAnimationFrame(() => {
                    setTimeout(() => {
                         const activityHeight = recentActivityContainer.offsetHeight;
                         
                         // Use CSS custom property to sync heights instead of inline styles
                         if (activityHeight > 0) {
                              // Set CSS variable for height synchronization
                              document.documentElement.style.setProperty('--news-widget-height', `${activityHeight}px`);
                              
                              // Ensure flexbox is working correctly
                              newsWidget.classList.add('news-widget-synced');
                              newsResults.classList.add('news-results-synced');
                         }
                    }, 200);
               });
          });
...
// Handle PWA app shortcuts
function handlePWAShortcuts() {
     const urlParams = new URLSearchParams(window.location.search);
     const action = urlParams.get('action');
     const tab = urlParams.get('tab');
     
     if (action === 'add-expense' && document.getElementById('add-expense-btn')) {
          setTimeout(() => {
               document.getElementById('add-expense-btn').click();
          }, 500);
     } else if (action === 'add-task' && document.getElementById('add-daily-task-btn')) {
          setTimeout(() => {
...
function setupOfflineDetection() {
     ...
     window.addEventListener('online', () => {
          updateOfflineStatus();
          showToast(translations[currentLang]?.offlineMessage || 'Connection restored', 'success');
     });
     
     window.addEventListener('offline', () => {
          updateOfflineStatus();
          showToast(translations[currentLang]?.offlineMessage || 'You are offline', 'warning');
     });
