          ...
          requestAnimationFrame(() => {
               requestAnimationFrame(() => {
                    setTimeout(() => {
                         ...
                         const toolbarHeight = newsToolbar.offsetHeight;
                         const resultsPadding = 24; // p-3 = 12px top + 12px bottom
                         
                         // Calculate available height for news results - use full height
                         const availableHeight = activityHeight - toolbarHeight - resultsPadding;
                         
                         // Set both height and maxHeight to ensure full height
                         if (availableHeight > 0) {
                              newsResults.style.height = `${availableHeight}px`;
                              newsResults.style.maxHeight = `${availableHeight}px`;
                              newsResults.style.minHeight = `${availableHeight}px`;
                              newsWidget.style.height = `${activityHeight}px`;
                              newsWidget.style.minHeight = `${activityHeight}px`;
                              
                              // Force reflow to ensure styles are applied
                              void newsWidget.offsetHeight;
                         }
                    }, 150);
               });
          });
